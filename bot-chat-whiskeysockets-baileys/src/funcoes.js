...
import { sock, clearSock } from './sockGlobal.js';
...
async function pegarDados(numero) {
  const { data, error } = await supabase
    .from('leads')
    .select('id, historico, num_mensagens, qualidade')
    .eq('numero', numero);
...
async function atualizarConexao(novoStatus) {
  const configs = await pegarConfigs();
  configs.conectado = novoStatus;
  await atualizarConfigs(configs);
}

async function escutarQrCode(startSession) {
...
export { responderPergunta, pegarLeads, atualizarLeadsFrias, realtimeSupabase, deleteFolder, escutarQrCode, atualizarConfigs, pegarConfigs, atualizarConexao };