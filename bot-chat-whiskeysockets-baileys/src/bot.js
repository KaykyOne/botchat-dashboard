...
import { responderPergunta, realtimeSupabase, atualizarConfigs, pegarConfigs, atualizarConexao } from './funcoes.js';
import axios from 'axios';
import fazedorDeQrCode from 'qrcode-terminal';
// import cron from 'node-cron';
// import fs from 'fs';
import dotenv from 'dotenv';
dotenv.config();
import { sock, clearSock, setSock } from './sockGlobal.js';
...
async function atualizarQrCode(novoQr) {
    const configs = await pegarConfigs();
    configs.qrCode = novoQr;
...
async function startSession() {
    ...
    setSock(makeWASocket({ printQRInTerminal: false, auth: state }));
...
async function startBot() {
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (qr) {
            console.log(qr);
            fazedorDeQrCode.generate(qr, { small: true });
            qrCode = qr;
            atualizarQrCode(qr);
        }
...
export { startBot, enviarMensagem, startSession };