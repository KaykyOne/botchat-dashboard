...
export default function Header({ item, buscar }: HeaderProps) {
  const router = useRouter()

  const [novaQualidade, setNovaQualidade] = useState<string>();
  const [iaAtiva, setIaAtiva] = useState<boolean>();
  ...
  return (
    <div className='w-full flex gap-2 p-5 bg-gradient-to-r from-neutral-900 via-neutral-800 to-neutral-900 items-center border-b border-neutral-700/50 backdrop-blur-sm shadow-2xl justify-between z-10'>
      ...
      <button className='bg-red-900 flex justify-center items-center gap-3 py-2 px-6 rounded-lg cursor-pointer hover:bg-red-700 transition-all duration-300' onClick={() => router.push('/')}>
        Sair
